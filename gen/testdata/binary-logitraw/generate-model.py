#!/usr/bin/env python
"""Generate a binary:logitraw model to test"""
...
NUM_ROUNDS = 500
DEFAULT_XGB_PARAMS = {
    # binary:logitraw is a logistic objective but outputs the raw margin with no
    # sigmoid, and (matching XGBoost) uses base_score directly as the raw
    # intercept rather than as a probability. 0.7 is an arbitrary non-default
    # value that exercises a nonzero raw intercept on the no-sigmoid output path.
...
feature_names = list(data.feature_names)  # pylint: disable=no-member
